function [em,emedio,ecmedio,r,fsuma] = parabolaf(m,x,y)
em=zeros(1,m);
emediosum=0;
ecmediosum=0;
num=0;
...
for k=1:m
    %disp (k);
    ymedio=(sum(y))/m;
    em(1,k) = abs(-0.1179 +1.4347*x(1,k)+2.9228*(x(1,k).^2)-y(1,k));
     num = num +(-0.1179 +1.4347*x(1,k)+2.9228*(x(1,k).^2)-ymedio)^2;
